    ...
    ui(new Ui::cartWidget)
{
    ...
    l2->setGeometry(QRect(1100,900,150,50));
...
void cartWidget::clickRm(QWidget* contentPd)
{
    ...
    if(QMessageBox::information(this,"","Bạn có chắc xóa sản phẩm này ra khỏi giỏ hàng",QMessageBox::Yes |QMessageBox::No)==QMessageBox::No) return;
    auto  p = Data.getPoiter(contentPd->objectName().toStdString());
    Data.remove(p);
    save(Data,username.toStdString());
