...
void addPd::updateKind(){
    ...
    id = id.substr(0,2) + (ui->kind1->currentIndex() < 10 ? "0":"" ) +  std::to_string(ui->kind1->currentIndex())  +  id.substr(4,id.length()-4);
...
void addPd::clickAddPhoto()
{
    ...
    ui->photoPd->setPixmap(photoPd.scaled(200,200,Qt::KeepAspectRatio));
}
